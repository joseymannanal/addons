...
class account_move(models.Model):
            ...
            if res_config:
                if res_config.tax_discount_policy == 'tax':
                    if rec.discount_type == 'line':
                        ...
                        rec.amount_total = rec.amount_tax + rec.amount_untaxed
            if rec.tax_discount_policy == True:
                sums = 0.00
                if rec.discount_type == 'line':
                    total = 0
                    if self._context.get('default_type') == 'out_invoice' :
    ...
    discount_amt = fields.Float(string='- Total Discount', readonly=True, compute='_compute_amount')
    ...
    discount_amt_line = fields.Float(compute='_compute_amount', string='- Total Discount', digits='Discount', store=True, readonly=True)
                ...
                if rec.tax_discount_policy == True:
                    if rec.discount_type == 'line':
                        ...
                        if rec.discount_method == 'fix':
                            ...
                            final_discount = (rec.discount_amt*base_line.price_subtotal)/100
